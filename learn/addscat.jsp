...
rs=st.executeQuery("select * from category where catid='"+catid+"'");
 if(rs.next()){
			%>
			<form action="addscat1.jsp" method="post" name="f1">
			<table><h3>
<tr>
  <td>Category ID:</td>
  <td><input type="hidden" name="catid" value="<%=rs.getString(1)%>"><%=rs.getString(1)%></td></tr>
<tr>
  <td>Category Name: </td>
  <td><input type="hidden" name="catname" value="<%=rs.getString(2)%>"><%=rs.getString(2)%></td></tr>
<tr>
  <td>Sub-category Name:</td>
  <td><input type="text" name="scatname" style="border-color:#000099; border-style:groove"></td></tr>
