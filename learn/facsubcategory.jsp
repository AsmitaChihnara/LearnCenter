...
<html lang="en">
<head>
<title>Learn Center | Sub-Category</title>
<meta charset="utf-8">
...
function verify1(){
 if(document.f2.t6.value==""){
    alert("Select one.");
	//document.f2.t6.focus();
	return false;
 }else 
	return true;
}
    ...
    <section id="content">
      <div class="box1">
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
              <h2 class="pad_bot1">faculty page </h2>
            </div>
           <h3>&nbsp;</h3>
                <form id="search" action="facsearch.jsp" method="post" name="f">
                      <input type="text" style="border-color:#000099; border-style:groove;margin:10px" name="ser" placeholder="Search Book"><input type="submit" value="search" onClick="return verify1()"></form>
			 <form action="facbook.jsp" method="get" name="f2">
			...
			<%
			scat=request.getParameter("t5");
			session.setAttribute("scatid",scat);
			ResultSet rs=st.executeQuery("select * from book where scatid='"+scat+"'");
			int cnt=0;
			while(rs.next()){
			cnt++;
			%>
						 <br><input type="radio" name="t6" value="<%=rs.getString(1)%>"><%=rs.getString(2)%>
			...
			<%}if(cnt!=0){%>
			 ...
			 <%}
			else{%><h5>No book available.<br></h5><%}%>
			 &nbsp;&nbsp;<a href="fachome.jsp" class="button"><span><span>Go Back</span></span></a>
			</form>
			<br><a href="facaddbook.jsp?scatid=<%=scat%>" class="button"><span><span>Add new book</span></span></a><br>
			<%
			 try{
...
%>
           ...
           <article class="col2 pad_left2">
            <div class="pad_left1">
              ...
              <p><h3><%=(String)session.getAttribute("name")%><br>Faculty</h3><br>
			   <a href="faceditprofile.jsp" class="button"><span><span>Edit Profile</span></span></a><a href="index.jsp" class="button"><span><span>Log out</span></span></a>
			    <br><br>
