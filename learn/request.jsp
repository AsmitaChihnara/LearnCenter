        ...
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
			  ...
			  <h4>Requests:</h4>
                <div>
				<%! int emp,fac,read,staf; %>
				<%
				try{
				emp=0;fac=0;read=0;staf=0;
				ResultSet rs=st.executeQuery("select * from login where status='n'");
				while(rs.next()){
				emp++;
				if(rs.getString("type").equalsIgnoreCase("employee"))
				staf++;
				else if(rs.getString("type").equalsIgnoreCase("faculty"))
				fac++;
				else if(rs.getString("type").equalsIgnoreCase("reader"))
				read++;
				}%>
				...
				<br>
			<h3>
				Total <%=emp%> requests are pending.<br><br><br>
				<ul class="list1">
              <li><a href="request1.jsp?req=faculty">Faculty requests:<%=fac%></a></li>
              <li><a href="request1.jsp?req=employee">Staff requests:<%=staf%></a></li>
			  <li><a href="request1.jsp?req=reader">Reader requests:<%=read%></a></li>
            </ul>
