...
<html lang="en">
<head>
<title>Learn Center | Requests</title>
<meta charset="utf-8">
...
session.setAttribute("req",type); %>
<h4><%=type%> Requests:</h4>
<%
try{ 
if(!type.equalsIgnoreCase("reader")){
rs=st.executeQuery("select a.* from employee a inner join login b on a.name=b.logname where b.type='"+type+"' and status='n'");
 while(rs.next()){
%>
 <div>
...
<tr><td>Designation:</td><td><%=type%></td></tr>
...
<a href="accept.jsp?ename=<%=rs.getString(2)%>" class="button"><span><span>Accept</span></span></a><a href="deny.jsp?ename=<%=rs.getString(2)%>" class="button"><span><span>Deny</span></span></a>
<br><br>
<% }
}
else{
rs=st.executeQuery("select a.* from reader a inner join login b on a.name=b.logname where b.type='"+type+"' and status='n'");
 while(rs.next()){
%>
 <div>
<figure class="left marg_right1"><img src="images/page4_img1.jpg" alt=""></figure>
...
<tr><td>Current Address:</td><td><%=rs.getString(9)%></td></tr>
<tr><td>Permanent Address:</td><td><%=rs.getString(10)%></td></tr>
</table><br>
<!--input type="hidden" name="editid" value=rs.getString(1)-->
<a href="accept.jsp?ename=<%=rs.getString(2)%>" class="button"><span><span>Accept</span></span></a><a href="denyread.jsp?ename=<%=rs.getString(2)%>" class="button"><span><span>Deny</span></span></a>
<br><br>
<% }
}
